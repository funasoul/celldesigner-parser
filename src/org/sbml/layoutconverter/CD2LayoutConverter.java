...
public class CD2LayoutConverter extends BaseLayoutConverter {
   ...
   *         the JAXB exception
   */
  public CD2LayoutConverter(File file) throws XMLStreamException, IOException,
    JAXBException {
    super(file);
    init(file);
  }
   ...
   *         the JAXB exception
   */
  public CD2LayoutConverter(File file, String outputFileName)
   ...
   *         the JAXB exception
   */
  public CD2LayoutConverter(File file, boolean defaultCompartment,
   ...
   *         the JAXB exception
   */
  public CD2LayoutConverter(File file, ApplicationOption options)
    throws XMLStreamException, IOException, JAXBException {
    super(file, options);
  ...
  public void init(File file) throws IOException, JAXBException,
    ...
    if (!SBMLUtil.isLevelAndVersionMatchWithCD(file))
      throw new IOException(
        "Level and Version does not match the CellDesigner defaults Level "
          + SBMLUtil.DEFAULT_CELLDESIGNER_SBML_LEVEL + " Version "
  ...
  public void convertCompartmentsToLayout(List<CompartmentAliasWrapper> cawList) {
    ...
    for (CompartmentAliasWrapper caw : cawList) {
      CompartmentGlyph cg = layout.createCompartmentGlyph("CompartmentGlyph_"
        + caw.getId());
  ...
  public void convertReactionsToLayout(List<ReactionWrapper> rList) {
    for (ReactionWrapper rw : rList) {
      ...
      ListOf<SpeciesReferenceGlyph> srgList = createSpeciesReferenceGlyph(rg,
        rw);
      ...
      Dimensions dim = new Dimensions(SBMLUtil.DEFAULT_SBML_LEVEL,
      ...
      if (brsList.size() == 1 && prsList.size() == 1) {
        ...
        SpeciesReferenceGlyph srg1 = srgList.get("srGlyphReactant_"
          + rg.getReaction() + "_" + br.getSpecies());
        SpeciesReferenceGlyph srg2 = srgList.get("srGlyphProduct_"
          + rg.getReaction() + "_" + bp.getSpecies());
        Point startPoint = LayoutUtil.createAdjustedPoint(srg1,
          br.getLinkAnchor().getPosition());
        Point endPoint = LayoutUtil.createAdjustedPoint(srg2,
          bp.getLinkAnchor().getPosition());
        Point reactantPoint = LayoutUtil.createCenterPoint(srg1);
        Point productPoint = LayoutUtil.createCenterPoint(srg2);
        lsList = LayoutUtil.createListOfLineSegment(startPoint, endPoint,
          reactantPoint, productPoint, editPointList, rectangleIndex);
        ...
        SpeciesReferenceGlyph srg1 = srgList.get("srGlyphReactant_"
          + rg.getReaction() + "_" + br1.getSpecies());
        SpeciesGlyph sg1 = srg1.getSpeciesGlyphInstance();
        Point startPoint = LayoutUtil.createAdjustedPoint(sg1,
          br1.getLinkAnchor().getPosition());
        BaseReactant br2 = brsList.get(1);
        SpeciesReferenceGlyph srg2 = srgList.get("srGlyphReactant_"
          + rg.getReaction() + "_" + br2.getSpecies());
        SpeciesGlyph sg2 = srg2.getSpeciesGlyphInstance();
        Point endPoint1 = LayoutUtil.createAdjustedPoint(sg2,
          br2.getLinkAnchor().getPosition());
        BaseProduct bp1 = prsList.get(0);
        SpeciesReferenceGlyph srg3 = srgList.get("srGlyphProduct_"
          + rg.getReaction() + "_" + bp1.getSpecies());
        SpeciesGlyph sg3 = srg3.getSpeciesGlyphInstance();
        Point endPoint2 = LayoutUtil.createAdjustedPoint(sg3,
          bp1.getLinkAnchor().getPosition());
        ...
        lsList = LayoutUtil.createListOfLineSegment(startPoint, endPoint1,
          endPoint2, reactantPoint1, reactantPoint2, productPoint1,
          editPointList, num0, num1, num2, rw.getReactionType(),
          rw.getEditPoints().getTShapeIndex());
        int reactant1 = num0;
        ...
        SpeciesReferenceGlyph srg2 = srgList.get("srGlyphProduct_"
          + rg.getReaction() + "_" + bp1.getSpecies());
        Point endPoint1 = LayoutUtil.createAdjustedPoint(srg2,
          bp1.getLinkAnchor().getPosition());
        BaseProduct bp2 = prsList.get(1);
        SpeciesReferenceGlyph srg3 = srgList.get("srGlyphProduct_"
          + rg.getReaction() + "_" + bp2.getSpecies());
        Point endPoint2 = LayoutUtil.createAdjustedPoint(srg3,
          bp2.getLinkAnchor().getPosition());
        Point reactantPoint = LayoutUtil.createCenterPoint(srg1);
        ...
        int num2 = rw.getEditPoints().getNum2();
        lsList = LayoutUtil.createListOfLineSegment(startPoint, endPoint1,
          endPoint2, reactantPoint, productPoint1, productPoint2,
          editPointList, num0, num1, num2, rw.getReactionType(),
          rw.getEditPoints().getTShapeIndex());
        int reactant1 = num0;
        int reactant2 = num0 + num1 + 1;
        ...
        curve = srg1.getCurve();
        int tshapeIndex = rw.getEditPoints().getTShapeIndex();
        LineSegment ls = (LineSegment) curve.getCurveSegment(curve.getCurveSegmentCount()
          - tshapeIndex - 1);
      ...
      if (rw.isSetModifier()) {
        for (Modification m : rw.getListOfModification()) {
          ...
          SpeciesReferenceGlyph srg = rg.getSpeciesReferenceGlyph("srGlyphModifier_"
            + rg.getReaction() + "_" + m.getModifiers());
          ...
          Point startPoint = LayoutUtil.createAdjustedPoint(sg,
            lt.getLinkAnchor().getPosition());
          List<LineSegment> lsList2 = LayoutUtil.createListOfLineSegment(
            startPoint, centerPoint, modifierPoint, centerPoint, editPointList);
      ...
      for (ReactantLink link : rlList) {
        ...
        SpeciesReferenceGlyph srg = srgList.get("srGlyphReactant_"
          + rg.getReaction() + "_" + link.getReactant());
        SpeciesGlyph sg = srg.getSpeciesGlyphInstance();
        String position = link.getLinkAnchor() != null ? link.getLinkAnchor()
                                                             .getPosition()
          : "INACTIVE";
        ...
        if (lineType.equals("Curve")) {
          lsList2 = LayoutUtil.createListOfBezier(startPoint, endPoint,
            lsList.get(rectangleIndex).getStart());
        } else if (rw.getReactionType().equals("HETERODIMER_ASSOCIATION")) {
          int num0 = rw.getEditPoints().getNum0();
          int num1 = rw.getEditPoints().getNum1();
          int tshapeIndex = rw.getEditPoints().getTShapeIndex();
          Point linkPoint = LayoutUtil.createLinkPoint(
            lsList.get(num0 + 1 + num1 + 1 + tshapeIndex).getStart(), endPoint);
          lsList2 = LayoutUtil.createListOfLineSegment(startPoint, linkPoint,
            editPointList);
        } else if (rw.getReactionType().equals("DISSOCIATION")
          || rw.getReactionType().equals("TRUNCATION")) {
          int num0 = rw.getEditPoints().getNum0();
          int tshapeIndex = rw.getEditPoints().getTShapeIndex();
          Point linkPoint = LayoutUtil.createLinkPoint(
            lsList.get(num0 - tshapeIndex).getStart(), endPoint);
          lsList2 = LayoutUtil.createListOfLineSegment(startPoint, linkPoint,
            editPointList);
        } else {
          Point linkPoint = LayoutUtil.createLinkPoint(
            lsList.get(rectangleIndex).getStart(), endPoint);
          lsList2 = LayoutUtil.createListOfLineSegment(startPoint, linkPoint,
            editPointList);
        }
        Curve curve = srg.createCurve();
      ...
      for (ProductLink link : plList) {
        String lineType = link.getLine().getType();
        SpeciesReferenceGlyph srg = srgList.get("srGlyphProduct_"
          + rg.getReaction() + "_" + link.getProduct());
        SpeciesGlyph sg = srg.getSpeciesGlyphInstance();
        String position = link.getLinkAnchor() != null ? link.getLinkAnchor()
                                                             .getPosition()
          : "INACTIVE";
        ...
        if (lineType.equals("Curve")) {
          lsList2 = LayoutUtil.createListOfBezier(startPoint, endPoint,
            lsList.get(rectangleIndex + 1).getEnd());
        } else if (rw.getReactionType().equals("HETERODIMER_ASSOCIATION")) {
          int num0 = rw.getEditPoints().getNum0();
          int num1 = rw.getEditPoints().getNum1();
          int tshapeIndex = rw.getEditPoints().getTShapeIndex();
          Point linkPoint = LayoutUtil.createLinkPoint(
            lsList.get(num0 + 1 + num1 + 1 + tshapeIndex).getEnd(), startPoint);
          lsList2 = LayoutUtil.createListOfLineSegment(linkPoint, endPoint,
            editPointList);
        } else if (rw.getReactionType().equals("DISSOCIATION")
          || rw.getReactionType().equals("TRUNCATION")) {
          int num0 = rw.getEditPoints().getNum0();
          int tshapeIndex = rw.getEditPoints().getTShapeIndex();
          Point linkPoint = LayoutUtil.createLinkPoint(
            lsList.get(num0 - tshapeIndex).getEnd(), startPoint);
          lsList2 = LayoutUtil.createListOfLineSegment(linkPoint, endPoint,
            editPointList);
        } else {
          Point linkPoint = LayoutUtil.createLinkPoint(
            lsList.get(rectangleIndex + 1).getEnd(), startPoint);
          lsList2 = LayoutUtil.createListOfLineSegment(linkPoint, endPoint,
            editPointList);
        }
        Curve curve = srg.createCurve();
      ...
      for (SpeciesReferenceGlyph srg : srgList) {
        if (srg.getSpeciesReferenceRole() == SpeciesReferenceRole.PRODUCT) {
          CurveSegment cs = (LineSegment) srg.getCurve()
   ...
   * @return the list of
   */
  public ListOf<SpeciesReferenceGlyph> createSpeciesReferenceGlyph(
    ReactionGlyph rg, ReactionWrapper rw) {
    ...
    for (BaseReactant br : baseReactantList) {
      SpeciesReferenceGlyph srg = rg.createSpeciesReferenceGlyph("srGlyphReactant_"
        + rg.getReaction() + "_" + br.getSpecies());
      if (SBMLUtil.containsReferencingSpecies(reaction.getListOfReactants(),
        br.getSpecies()))
    ...
    for (BaseProduct bp : baseProductList) {
      SpeciesReferenceGlyph srg = rg.createSpeciesReferenceGlyph("srGlyphProduct_"
        + rg.getReaction() + "_" + bp.getSpecies());
      if (SBMLUtil.containsReferencingSpecies(reaction.getListOfProducts(),
        bp.getSpecies()))
    ...
    if (rw.isSetModifier()) {
      for (Modification m : modificationList) {
        if (m.getType().contains("BOOLEAN"))
          continue;
        SpeciesReferenceGlyph srg = rg.createSpeciesReferenceGlyph("srGlyphModifier_"
          + rg.getReaction() + "_" + m.getModifiers());
        srg.setSpeciesReference(m.getModifiers());
        srg.setSpeciesGlyph("SpeciesGlyph_" + m.getAliases());
        srg.setBoundingBox(srg.getSpeciesGlyphInstance().getBoundingBox()
                              .clone());
