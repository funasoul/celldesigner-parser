...
public class LayoutUtil {
   ...
   *         TODO
   */
  public static Point getEditPointPosition(SpeciesGlyph origin,
    SpeciesGlyph axis1, SpeciesGlyph axis2, Point2D.Double editPoint) {
   ...
   *         TODO
   */
  public static Point getEditPointPosition(Point origin, Point axis1,
    Point axis2, Point2D.Double editPoint) {
   ...
   * @return the double
   */
  public static double dotProduct(Point startPoint, Point endPoint1,
    Point endPoint2) {
    Point vec1 = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
      SBMLUtil.DEFAULT_SBML_VERSION);
    vec1.setX(endPoint1.getX() - startPoint.getX());
    vec1.setY(endPoint1.getY() - startPoint.getY());
    Point vec2 = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
      SBMLUtil.DEFAULT_SBML_VERSION);
   ...
   *         TODO
   */
  public static Point createAdjustedPoint(double x, double y, double width,
    double height, String direction) {
    Point point = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
      SBMLUtil.DEFAULT_SBML_VERSION);
    point.setZ(0);
   ...
   */
  public static List<LineSegment> createListOfBezier(Point startPoint,
    Point endPoint, Point basePoint) {
    List<LineSegment> lsList = new ArrayList<LineSegment>();
    CubicBezier bezier = new CubicBezier(SBMLUtil.DEFAULT_SBML_LEVEL,
      SBMLUtil.DEFAULT_SBML_VERSION);
    bezier.setStart(startPoint.clone());
    bezier.setEnd(endPoint.clone());
    bezier.setBasePoint1(calculateBasepointQuadraticToCubic(startPoint,
      basePoint));
   ...
   * @return the point
   */
  public static Point calculateBasepointQuadraticToCubic(Point startPoint,
    Point quadBasePoint) {
    Point point = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
      SBMLUtil.DEFAULT_SBML_VERSION);
    point.setX(startPoint.getX() + 2d / 3d
      * (quadBasePoint.getX() - startPoint.getX()));
    point.setY(startPoint.getY() + 2d / 3d
      * (quadBasePoint.getY() - startPoint.getY()));
    return point;
  }
   ...
   *         TODO
   */
  public static List<LineSegment> createListOfLineSegment(Point startPoint,
    Point endPoint1, Point endPoint2, Point speciesPoint1, Point speciesPoint2,
    Point speciesPoint3, List<Point2D.Double> editPointList, int num0,
    int num1, int num2, String type, int tshapeIndex) {
    List<LineSegment> lineList = new ArrayList<LineSegment>();
    Point editPoint = getEditPointPosition(speciesPoint1, speciesPoint2,
      speciesPoint3, editPointList.get(editPointList.size() - 1));
    if (num0 > 0) {
      ...
      LineSegment ls = new LineSegment(SBMLUtil.DEFAULT_SBML_LEVEL,
        SBMLUtil.DEFAULT_SBML_VERSION);
    ...
    if (num1 > 0) {
      ...
      if (type.equals("DISSOCIATION") || type.equals("TRUNCATION")) {
        subLineList = createListOfLineSegment(editPoint, endPoint1, editPoint,
          endPoint1, subList);
      } else {
        Collections.reverse(subList);
        subLineList = createListOfLineSegment(endPoint1, editPoint, editPoint,
          endPoint1, subList);
      }
      lineList.addAll(lineList.size(), subLineList);
    } else {
    ...
    if (num2 > 0) {
      List<Point2D.Double> subList = editPointList.subList(num0 + num1, num0
        + num1 + num2);
      List<LineSegment> subLineList = createListOfLineSegment(editPoint,
        endPoint2, editPoint, endPoint2, subList);
      lineList.addAll(lineList.size(), subLineList);
    } else {
      LineSegment ls = new LineSegment(SBMLUtil.DEFAULT_SBML_LEVEL,
        SBMLUtil.DEFAULT_SBML_VERSION);
   ...
   */
  public static List<LineSegment> createListOfLineSegment(Point startPoint,
    Point endPoint, Point reactantPoint, Point productPoint,
    List<Point2D.Double> editPointList, int rectangleIndex) {
    List<LineSegment> lineList = createListOfLineSegment(startPoint, endPoint,
      reactantPoint, productPoint, editPointList);
    LineSegment l1 = lineList.get(rectangleIndex);
    LineSegment l2 = new LineSegment(SBMLUtil.DEFAULT_SBML_LEVEL,
      SBMLUtil.DEFAULT_SBML_VERSION);
   ...
   * @return the list
   */
  public static List<LineSegment> createListOfLineSegment(Point vector1,
    Point vector2, List<Point2D.Double> editPointList) {
    return createListOfLineSegment(vector1, vector2, vector1, vector2,
      editPointList);
  }
   ...
   */
  public static List<LineSegment> createListOfLineSegment(Point startPoint,
    Point endPoint, Point vector1, Point vector2,
    List<Point2D.Double> editPointList) {
    List<LineSegment> lineList = new ArrayList<LineSegment>();
    Point perpPoint = createPerpendicularPoint(vector1, vector2);
    LineSegment line = new LineSegment(SBMLUtil.DEFAULT_SBML_LEVEL,
      SBMLUtil.DEFAULT_SBML_VERSION);
    line.setStart(startPoint.clone());
    for (int i = 0; i < editPointList.size(); i++) {
      Point point = getEditPointPosition(vector1, vector2, perpPoint,
        editPointList.get(i));
      line.setEnd(point.clone());
      lineList.add(line);
      line = new LineSegment(SBMLUtil.DEFAULT_SBML_LEVEL,
        SBMLUtil.DEFAULT_SBML_VERSION);
   ...
   * @return the string
   */
  public static List<String> editPointListToStringList(
    List<Point2D.Double> editPointList) {
   ...
   *         TODO
   */
  public static Point createCenterPoint(double x, double y, double width,
    double height) {
   ...
   * @return the list
   */
  public static List<Point2D.Double> convertEditPointsToProportion(
    Point startPoint, Point vector1, Point vector2,
    List<Point2D.Double> editPoints) {
    ...
    for (Point2D.Double point : editPoints) {
      ...
      Point origin = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
        SBMLUtil.DEFAULT_SBML_VERSION);
      origin.setX(0);
      origin.setY(0);
      Point vec1 = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
      ...
      Point edit = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
        SBMLUtil.DEFAULT_SBML_VERSION);
      ...
      Point intersect = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
        SBMLUtil.DEFAULT_SBML_VERSION);
   ...
   */
  public static List<Point2D.Double> convertEditPointsToProportion(
    Point startPoint, Point endPoint, List<Point2D.Double> editPoints) {
    List<Point2D.Double> editPointList = new ArrayList<Point2D.Double>();
    Point perpPoint = createPerpendicularPoint(startPoint, endPoint);
    for (Point2D.Double point : editPoints) {
      Point2D.Double editPoint = new Point2D.Double();
      double length1 = getOrthogonalProjectionLengthFromEditPoint(startPoint,
        endPoint, point);
      editPoint.x = length1 / getLength(startPoint, endPoint);
      double length2 = getOrthogonalProjectionLengthFromEditPoint(startPoint,
        perpPoint, point);
   ...
   */
  public static double getOrthogonalProjectionLengthFromEditPoint(
    Point startPoint, Point endPoint, Point2D.Double editPoint) {
    Point edit = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
      SBMLUtil.DEFAULT_SBML_VERSION);
    edit.setX(editPoint.getX());
    edit.setY(editPoint.getY());
    return getOrthogonalProjectionLengthFromEditPoint(startPoint, endPoint,
      edit);
  }
   ...
   */
  public static double getOrthogonalProjectionLengthFromEditPoint(
    Point startPoint, Point endPoint, Point editPoint) {
   ...
   * @return the double
   */
  public static double angleOf2Vector(Point startPoint, Point endPoint1,
    Point endPoint2) {
   ...
   */
  public static double getOrthogonalProjection(Point startPoint,
    Point endPoint, double radian) {
  ...
  public static Point createLinkPoint(Point nearestPoint, Point centerPoint) {
    Point p = new Point(SBMLUtil.DEFAULT_SBML_LEVEL,
      SBMLUtil.DEFAULT_SBML_VERSION);
    double proportion = 3 / 4d;
    p.setX(nearestPoint.getX() + (centerPoint.getX() - nearestPoint.getX())
      * proportion);
    p.setY(nearestPoint.getY() + (centerPoint.getY() - nearestPoint.getY())
      * proportion);
    return p;
  }
   ...
   */
  // TODO determine the species position depending on how each glyphs are layed
  // out
  public static String getPositionToCompartment(SpeciesGlyph sg,
    CompartmentGlyph cg) {
    return "inside";
  }
