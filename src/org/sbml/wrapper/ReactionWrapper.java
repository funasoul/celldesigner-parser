...
public class ReactionWrapper extends Reaction {
  ...
  public ReactionWrapper(Reaction reaction, ModelWrapper modelWrapper) {
    ...
    this.name = reaction.getName(); // same as
  ...
  void setAnnotations() {
    ...
    if (annotation.getExtension().getListOfModification() != null)
      this.modificationList = annotation.getExtension().getListOfModification()
                                        .getModification();
    if (annotation.getExtension().getBaseReactants() != null) {
      baseReactants = annotation.getExtension().getBaseReactants()
                                .getBaseReactant();
      for (BaseReactant br : baseReactants) {
        if (br.getLinkAnchor() == null
          || br.getLinkAnchor().getPosition() == null) {
    ...
    if (annotation.getExtension().getBaseProducts() != null) {
      baseProducts = annotation.getExtension().getBaseProducts()
                               .getBaseProduct();
      for (BaseProduct bp : baseProducts) {
        if (bp.getLinkAnchor() == null
          || bp.getLinkAnchor().getPosition() == null) {
          LinkAnchor anchor = new LinkAnchor();
          anchor.setPosition("INACTIVE");
    ...
    if (annotation.getExtension().getListOfReactantLinks() != null)
      reactantLinks = annotation.getExtension().getListOfReactantLinks()
                                .getReactantLink();
    else
      reactantLinks = new ArrayList<ReactantLink>();
    if (annotation.getExtension().getListOfProductLinks() != null)
      productLinks = annotation.getExtension().getListOfProductLinks()
                               .getProductLink();
    else
    ...
    if (connectScheme != null && connectScheme.getRectangleIndex() != null)
      rectangleIndex = Integer.valueOf(annotation.getExtension()
                                                 .getConnectScheme()
                                                 .getRectangleIndex());
   ...
   * (non-Javadoc)
   * @see
   * org.sbml.sbml.level2.version4.OriginalReaction#setName(java.lang.String)
   */
  ...
  public void setReactantLinkLineType(String id, Type type) {
    for (ReactantLink link : reactantLinks) {
      if (link.getReactant().equals(id))
        link.getLine().setType(
          LayoutUtil.convertLineTypeToCDString(type.name()));
    }
  }
   ...
   *         TODO
   */
  public List<SpeciesReferenceWrapper> createReactantWrapperList(
    List<SpeciesReference> srList) {
    List<SpeciesReferenceWrapper> srwList = new ArrayList<SpeciesReferenceWrapper>(
      srList.size());
   ...
   *         TODO
   */
  public List<SpeciesReferenceWrapper> createProductWrapperList(
    List<SpeciesReference> srList) {
    List<SpeciesReferenceWrapper> srwList = new ArrayList<SpeciesReferenceWrapper>(
      srList.size());
    for (SpeciesReference sr : srList)
   ...
   *         TODO
   */
  public List<ModifierSpeciesReferenceWrapper> createModifierWrapperList(
    List<ModifierSpeciesReference> msrList) {
    List<ModifierSpeciesReferenceWrapper> msrwList = new ArrayList<ModifierSpeciesReferenceWrapper>(
      msrList.size());
  ...
  public void setRectangleIndex(int index) {
    annotation.getExtension().getConnectScheme()
              .setRectangleIndex(String.valueOf(index));
